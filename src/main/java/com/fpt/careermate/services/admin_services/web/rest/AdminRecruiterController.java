...
public class AdminRecruiterController {
        ...
        @GetMapping("/filter")
        @Operation(summary = "Get recruiters by status with pagination", description = "Admin can filter recruiters by status (PENDING, ACTIVE, BANNED) with pagination and sorting. "
                        +
                        "Leave status empty to get all recruiters.")
        public ApiResponse<PageResponse<RecruiterApprovalResponse>> getRecruitersByStatus(
                        @Parameter(description = "Account status filter (PENDING, ACTIVE, BANNED, or empty for all)") @RequestParam(required = false) String status,
                        @Parameter(description = "Page number (0-based)") @RequestParam(defaultValue = "0") int page,
                        @Parameter(description = "Page size") @RequestParam(defaultValue = "10") int size,
                        @Parameter(description = "Sort field (e.g., companyName, createdAt)") @RequestParam(defaultValue = "id") String sortBy,
        ...
        @GetMapping("/search")
        @Operation(summary = "Search recruiters with filters", description = "Admin can search recruiters by company name, email, or username with status filter and pagination. "
                        +
        ...
        public ApiResponse<PageResponse<RecruiterApprovalResponse>> searchRecruiters(
                        @Parameter(description = "Search keyword (searches in company name, email, username)") @RequestParam(required = false) String search,
                        @Parameter(description = "Account status filter (PENDING, ACTIVE, BANNED, or empty for all)") @RequestParam(required = false) String status,
                        @Parameter(description = "Page number (0-based)") @RequestParam(defaultValue = "0") int page,
                        @Parameter(description = "Page size") @RequestParam(defaultValue = "10") int size,
                        @Parameter(description = "Sort field (e.g., companyName, createdAt)") @RequestParam(defaultValue = "id") String sortBy,
                ...
                log.info("Admin searching recruiters: search='{}', status={}, page={}, size={}", search, status, page,
                                size);
                return ApiResponse.<PageResponse<RecruiterApprovalResponse>>builder()
        ...
        @Operation(summary = "Get recruiter details by ID", description = "Admin can view detailed information of a specific recruiter including organization info")
        public ApiResponse<RecruiterApprovalResponse> getRecruiterById(
                        @Parameter(description = "Recruiter ID") @PathVariable int recruiterId) {
        ...
        @GetMapping("/pending")
        @Operation(summary = "[Legacy] Get all pending recruiter accounts", description = "Admin can view all recruiter accounts with PENDING status waiting for approval. "
                        +
        ...
        @GetMapping
        @Operation(summary = "[Legacy] Get all approved recruiters", description = "Admin can view all approved recruiters (accounts with RECRUITER role). "
                        +
        ...
        @Operation(summary = "Approve recruiter account", description = "Admin approves recruiter account: sets account status to ACTIVE and verification status to APPROVED. Role remains RECRUITER.")
        ...
        @Operation(summary = "Reject recruiter account", description = "Admin rejects recruiter account by setting account status to REJECTED and verification status to REJECTED. Data is preserved for audit purposes.")
        public ApiResponse<Void> rejectRecruiter(@PathVariable int recruiterId,
                ...
                return ApiResponse.<Void>builder()
                                .code(200)
                                .message("Recruiter account rejected. Status changed to REJECTED. Reason: "
                                                + (reason != null ? reason : "Not specified"))
                                .build();
        }

        @PutMapping("/account/{accountId}/ban")
        @Operation(summary = "Ban recruiter account", description = "Admin bans a recruiter account, preventing them from logging in or accessing the system")
        public ApiResponse<Void> banRecruiterAccount(@PathVariable int accountId,
                ...
                return ApiResponse.<Void>builder()
                                .code(200)
                                .message("Account banned successfully. Reason: "
                                                + (reason != null ? reason : "Not specified"))
                                .build();
        }

        @PutMapping("/account/{accountId}/unban")
        @Operation(summary = "Unban recruiter account", description = "Admin unbans a previously banned recruiter account, allowing them to access the system again")
